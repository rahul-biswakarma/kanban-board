...
const KanbanTask: React.FC<TaskPropsType> = (props) => {
	...
	function deleteTask(id: string, columnIndex: number) {
		...
		if (boardNo !== null) {
			const newTasks = newBoards[boardNo].columns[columnIndex].tasks.filter(
				(task: any) => task.id !== id
			);
			newBoards[boardNo].columns[columnIndex].tasks = newTasks;
			setBoards(newBoards);
					...
					<div className="flex justify-between flex-wrap">
						...
						<div className="flex justify-end items-center gap-[10px]">
							...
							<svg
								...
								strokeWidth={1.5}
								onClick={() => {
									deleteTask(task.id, props.columnIndex);
								}}
